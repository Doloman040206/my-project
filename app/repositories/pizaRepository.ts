...
export const pizaRepository: PizaRepo = {
  ...
  async findById(id: string) {
    const [rows] = await pool.execute<PizaData[] & RowDataPacket[]>(
      'SELECT * FROM piza WHERE id = ?',
      [id],
    );
    return rows[0] ?? null;
  },
  ...
  async update(id, patch) {
    ...
    if (patch.ingridients !== undefined) {
      fields.push('ingridients = ?');
      values.push(patch.ingridients);
    }
